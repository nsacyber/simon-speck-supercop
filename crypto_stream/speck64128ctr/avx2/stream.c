...
#include "stdio.h"
#include "stdlib.h"
#include "Speck64128AVX2.h"


int crypto_stream_speck64128ctr_avx2(unsigned char *out, unsigned long long outlen, const unsigned char *n, const unsigned char *k);
inline __attribute__((always_inline)) int Encrypt(unsigned char *out, u32 nonce[], u256 rk[], u32 key[], int numbytes);
int crypto_stream_speck64128ctr_avx2_xor(unsigned char *out, const unsigned char *in, unsigned long long inlen, const unsigned char *n, const unsigned char *k);
inline __attribute__((always_inline)) int Encrypt_Xor(unsigned char *out, const unsigned char *in, u32 nonce[], u256 rk[], u32 key[], int numbytes);
int ExpandKey(u32 K[], u256 rk[], u32 key[]);



int crypto_stream_speck64128ctr_avx2(
  unsigned char *out,
...
int crypto_stream_speck64128ctr_avx2_xor(
  unsigned char *out,
