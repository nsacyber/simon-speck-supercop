...
int crypto_stream_simon128192ctr_ref(
  unsigned char *out, 
...
{
  u64 i,nonce[2],K[3],key[69],x,y,t;
...
int crypto_stream_simon128192ctr_ref_xor(
  unsigned char *out, 
...
int ExpandKey(u64 K[],u64 key[])
{
  ...
  for(i=3;i<67;i++){
    key[i]=c^(z&1)^key[i-3]^ROR64(key[i-1],3)^ROR64(key[i-1],4); 
    z>>=1;
  }
  key[67]=c^key[64]^ROR64(key[66],3)^ROR64(key[66],4);
  key[68]=c^1^key[65]^ROR64(key[67],3)^ROR64(key[67],4);
