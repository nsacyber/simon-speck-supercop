//AVX512 C code for SPECK 128/192
...
#include <stdlib.h>
#include <string.h>
#include "Speck128192AVX512.h"

int crypto_stream_speck128192ctr_avx2(unsigned char *out, unsigned long long outlen, const unsigned char *n, const unsigned char *k);
inline __attribute__((always_inline)) int Encrypt(unsigned char *out, u64 nonce[], u512 rk[], u64 key[], int numbytes);
int crypto_stream_speck128192ctr_avx512_xor(unsigned char *out, const unsigned char *in, unsigned long long inlen, const unsigned char *n, const unsigned char *k);
...
int crypto_stream_speck128192ctr_avx512(
  unsigned char *out, 
...
{
  int i;
  u64 nonce[2],K[3],key[33],A,B,C,x,y;
...
int crypto_stream_speck128192ctr_avx512_xor(
  unsigned char *out, 
